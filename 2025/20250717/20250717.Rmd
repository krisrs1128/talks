...
(Screenshot from the Sun and Xia book)
...
These are analogous comparisons for the simulated microbiomes. Being able to
sample from the fitted mediation models can help with model goodness-of-fit
checking.
...
1. The study `r Citep(bib, "")` attempted an integrative analysis to discover
...
150 - 200 of the most abundant features.

1. We treat the microbiome as the mediators and the metabolome as the outcome.
This direction could be debated, but is consistent with the discussion in `r
Citep(bib, )`. 
...
Here we've visualized the samples using a multidimensional scaling (MDS) based
on _only_ the microbiome data. The size of the points reflects the metabolite
abundance. There is only a systematic association between metabolite abundance
and MDS coordinate for the outcomes with large indirect effects.

.center[
<img src="figures/indirect_effects_metabolites.png"/>
]
...
<img src="hurdle_model_indirect.png"/>
