...
\begin{align*}
\frac{1}{2} \sum_{t=0}^{1} \mathbb{E}\{Y(t, M(1)) - Y(t, M(0))\}\\
\frac{1}{2} \sum_{t'=0}^{1} \mathbb{E}\{Y(1, M(t')) - Y(0, M(t'))\}
\end{align*}

We may also want to know the pathwise indirect effect through mediator $k$:
\begin{align*}
\frac{1}{2} \sum_{t'=0}^{1} \mathbb{E}\{Y(t', M_k(1), M_{-k}(t')) - Y(t', M_k(0), M_{-k}(t'))\}
\end{align*}
...
example. 
.center[
<img src="figures/lasso_bootstrap_vis-1.png" width=900/>
]
...
```{r, results='asis', echo = FALSE}
PrintBibliography(bib, start = 13, end = 26)
```
