...
.center[
<img src="figures/contrast-simulators-1.png" width=700/>
]
...
* _Step 1 (Estimate)_: Train a simulator on the original data.

* _Step 2 (Alter)_: Define negative control taxa with no association with T1D.

* _Step 3 (Compare)_: Evaluate prediction performance of SPLS-DA applied to semisynthetic data with varying sample sizes and fractions of negative control taxa.
...
.center[
<img src="figures/splsda-correlations.png" width=900/>
]
...
1. We called this a *reliability check*, though this is not standard
terminology. The checks use the fact that simulations allow us to construct
hypothetical scenarios that we can never be sure about in real data.
...
The study (tk cite) used amplicon sequencing data to profile the bacterial,
viral, and fungal composition of the blood (tk check) samples from patients at a
hospital, including a subset who were experiencing sepsis.
...
multiple tables. With sepsis status as the response variable, the method outputs
...
These are the statistical details of our data generating mechanism.
...
It's relatively standard to compare $R^{2}$ or prediction performance when
trying to decide whether one model is more plausible than another.


Semisynthetic data can be used in a similar way, and it's especially powerful
when we want to reason about patterns of association that can't be captured by
regression alone.
...
psychiatric conditions, both in mouse models and in observational human studies
`r Citep(bib, c("morais2021gut", "bosch_gut_2022"))`.
...
Jo Handelsman (Dept. Plant Pathology) and Richie Davidson (Depts. Psychology,
Psychiatry, and Neuroscience) have been leading several studies to explore these
...
We re-analyzed a pilot study from 2021, which gathered data from (tk) healthy
participants and gathered microbiome data before and after a mindfulness
intervention.
...
microbiome and diet survey data `r Citep(bib, c("imai2010general", "sohn2019compositional"))`.
...
We can then simulate microbiome profiles assuming either:

1. Changes in the microbiome only result from diet changes.
1. Changes in the microbiome has no relationship with diet.
...
Here are profiles simulated according to these different pathways.
...
All the examples I discussed today (and more) can be reproduced in online
tutorials we've written to accompany our papers:

* Simulation for Microbiome Analysis [go.wisc.edu/wnj5p9](https://go.wisc.edu/wnj5p9)
* Generative Model: An Interdisciplinary Perspective [go.wisc.edu/ax73qb](https://go.wisc.edu/ax73qb)
...
1. By creating ground truth with which to analyze complex workflows, simulation
can help us to think critically about our data analysis. Recent developments
make data generation much more practical than it was in the past.

1. We need your help! As we build more simulation software, the most important
factor for adoption is a large collection of authentic and interesting examples.
If you have data or problems that could benefit from simulation, please reach
out.
