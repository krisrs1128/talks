...
.pull-left[
\begin{align*}
x_{i} \sim \Mult\left(N_{i}, \varphi^{-1}\left(z_{i}\right)\right) \\
...
\hat{V} &:= \text{PCA}\left(\hat{E}_1, \dots, \hat{E}_{n}\right) \in \reals^{N \times K}
\end{align*}
...
\hat{E}_i &= M_i - \hat{\alpha} T_i+\hat{\zeta}_M^{\top} X_i\\
\hat{V} &:= \text{PCA}\left(\hat{E}_1, \dots, \hat{E}_{n}\right) \in \reals^{N \times K}\\
Y &= \eta T + \beta^\top \left(\left(I - P_{T}\right)M V\right) + \zeta_Y
\end{align*}
...
```{r, results='asis', echo = FALSE}
PrintBibliography(bib, start = 16, end = 27)
```
