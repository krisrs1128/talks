...
visualization tutorial from the scanpy package `r Citep(bib, )`. Each point is
the UMAP embedding of a cell's high-dimensional gene expression data.
...
* Local distances should not be interpreted in the same way across all the clusters.
* Cells on the boundary of some of the clusters are neighbors with cells in clusters that are not the closest in the embedding space.
...
Here is an application to a more realistic hydra cell atlas dataset, which was
also used to study distortion in `r Citep(bib, "xia2024statistical")`. We fit
t-SNE with the perplexity hyperparameter set to 80. Points around the boundary
are collapsed, and between-cluster distances are exaggerated.
...
When we increase perplexity to 500, the clusters are more reliable, but samples
along the boundary of the visualization are in fact closer than they appear.
...
Our visualization approach can still fail when local neighborhood information
isn't enough. In this example, the two rings are overlapping in the original
data, but with centers further and further appart.
