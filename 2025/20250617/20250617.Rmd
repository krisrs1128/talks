...
The relationship between the microbiome and cancer is strong but complex.
...
1. Since in principle the microbiome could be altered, there is interest in
optimizing it to improve responses to cancer therapies.
...
.pull-left[
The study `r Citep(bib, "Castellarin2012")` found _F. nucleatum_ at 400X higher
levels in colorectal cancer vs. healthy tissue. It's believed that when this
bacteria binds to a cell's surface, it activates production of proinflammatory
cytokines that increase the chance of epithelial-to-mesenchymal transition.
]
...
In a mouse model of liver cancer, `r Citep(bib, "")` found that the microbiome
can produce metabolites that convert primary to secondary bile acids. The
primary bile acids up-regulate CXCL16 production, which protects against
metastasis -- essentially, the microbiome weakens this protection.
...
treatments `r Citep(bib, "Chalif20225")`.
]
...
checkpoint inhibitors `r Citep(bib, "Vetizou2015", "Sivan2015")`. This fact has
...
> Technically, such big data harmonization and integration is far from trivial, while "in-house" meethodologies, even when applied to the same datasets, may lead to nonoverlapping conclusions"

-- `r Citep(bib, "Culling2021")`
...
-- `r Citep(bib, "Gopalakrishnan2023")`
