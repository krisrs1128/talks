...
1. ComBat `r Citep(bib, "Johnson2006")` Linear Model.
1. CellAnova `r Citep(bib, "Zhang2024")` Clustering + Linear Model.
...
If sample $i$ is in batch $m$, then it is assumed to be drawn from:
...
- [**Notation**] Let $\*Y^1, ..., \*Y^M$ be measurements across $M$ biological samples. 
  - Let the first $M_0$ samples are controls. 
  - Each $\*Y^m \in \reals^{N_{m} \times G}$.
- [**Controls**] The control samples are assumed to be biologically comparable
...
\begin{align}
\bm{Y}^m &= \bm{C}^m(\mu + \bm{B}^m \bm{V}^\top + \bm{T}^m \bm{W}^\top) + \text{noise} \\\\
...
We want to estimate:
- $\mu$: Global mean
- $\*B^m \*V^\top$: Batch effects component  
- $\*T^m \*W^\top$: Real biological variation component
...
\begin{align}
\begin{pmatrix}
\bm{R}^1-\overline{\bm{{R}}}^0 \\\\
\vdots \\\\
\bm{{R}}^{M_0}-\overline{\bm{{R}}}^0
\end{pmatrix} \in \reals^{K_c M_0 \times G} \xrightarrow{\mathrm{SVD}} \bm{V} \in \mathbb{R}^{G \times K_b}
\end{align}
...
\begin{align}
\begin{bmatrix} \bm{R}^1 - \overline{\bm{R}} \\\\ \vdots \\\\ \bm{R}^M - \overline{\bm{R}} \end{bmatrix}(I - \bm{V}\bm{V}^\top) \xrightarrow{\text{SVD}} W
\end{align}

Then $\bm{T}^m = (\bm{R}^m - \overline{\bm{R}})\bm{W}$ are coordinates with respect to real
...
vs.
\begin{align}
\bm{Y} = \bm{C}\left(\bm{M} + \bm{X} \bm{W}^\top + \bm{B} \bm{V}^\top\right)
\end{align}
