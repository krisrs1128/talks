...
All of these models are variants of the Logistic Normal Multinomial (LNM) Model,
...
We incorporate the mediator path in an LNM model. For biological samples $i = 1,
\dots, N$ and replicates $i_{1}, \dots, i_{r}$, this looks like,

\begin{align*}
y_{i_r} &\sim \Mult\left(\text{Depth}_{i_r}, \varphi^{-1}\left(\eta_{i}\right)\right) \\
\eta_{i}&= \beta_0 + \beta_T t_i + \beta_X^T x_i + \beta_M^T m_{i}\\ 
...
comparison. The usual evaluation approaches are not sufficient,
...
This is the same plot, but without the spike at 0.
...
* Consider the problem of evaluating an LNM model,
...
\begin{align*}
y_{i} \sim \Mult\left(N_{i}, \exp{\beta_{T}x_{i}}\cdot \hat{p}_{i}^{\ast}\right)
\end{align*}
...
Thank you also to all Latent Structure Lab team members for valuable discussion.

Funds from the Wisconsin Alumni Research Fund are gratefully acknowledged.
