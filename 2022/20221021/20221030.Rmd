...
1. We resolve these difficulties by defining a semisynthetic simulator, following the ZINQ approach of `r Citep(bib, "zhao")`.
...
\begin{align*}
y_{i} \sim \Mult\left(N_{i}, \varphi^{-1}\left(\beta_{0} + \beta_{\text{Trt}}x_{i}\right)\right)
\end{align*}
where $\varphi\left(x\right) \propto\left(\exp{x_{1}}, \dots, \exp{x_{K}}\right)$.
...
\begin{align*}
y_{i} \sim \Mult\left(N_{i}, \varphi^{-1}\left(\beta_{0} + \beta_{\text{Trt}}x_{i}\right)\right) \\
\beta_{0} \sim \Gsn\left(0, I_{K}\right) \\
\beta_{\text{Trt}} := Threshold\left(\tilde{\beta}_{\text{Trt}}, 0.75\right)
\tilde{\beta}_{Trt} \sim \Gsn\left(0, I_{K}\right) \\
\end{align*}

<img src="figures/lnm-spherical.png" width=600/>

----

### Semisynthetic Setup

In the second, we use the exact same $\beta_{\text{Trt}}$, but this is used to
exponentially tilt samples from treatment,

\begin{align*}
y_{i} \sim \Mult\left(N_{i}, \exp{\beta_{Trt}x_{i}}\hat{p}_{i}^{\ast}\right)
\end{align*}
...
The purely synthetic simulation setup leads to overoptimistic power and false
sign rate estimates, compared to the semisynthetic setup.

<img src="figures/semisynthetic_comparison" width=700/>
