...
* Technologies like 16S and metagenomic sequencing have given us a detailed view into these systems
* An improved understanding of the microbiome has implications for human and environmental health
]
...
## Part 1: Visualization in Microbial Community Analysis

---

### Motivating Study (1 of 3)

1. In `r Citep(bib, "sankaran2019latent")`, we reanalyzed data from `r Citep(bib, "dethlefsen2011incomplete")`, which took longitudinal species count from 3 subjects (52 - 56 timepoints)
...
* In the Sankey diagram below, columns are models and rectangles are topics. 
...
# Part 2: Simulation in Microbial Community Analysis
...
2. The unblocked model also makes spurious associations, since subjects in some treatment groups might have large abundance of a certain bacteria (regardless of treatment assignment)
...
2. We include several other experimental design-related examples in the review paper `r Citep('sankaran2022')`.
...
1. Generative graphical models provide a language for clarifying the dependence relationships across measurements
2. Linked variables are dependent after controlling the influence from others
3. 
.center[
...
3. Compositional Intercepts from Baseline
...
Rather than describing individual models, I would like to describe how we approach model comparison. The difficulty is that standard evaluation approaches are not sufficient,

1. Prediction performance: Good prediction of future composition doesn’t guarantee effective inference of direct or indirect effects.
2. Traditional Simulation: Simulating from one of the assumed model structures gives it an unfair advantage.

---

### Zero-Inflated Quantiles (ZINQ)

1. We resolve these difficulties by defining a semisynthetic simulator, following the ZINQ approach of `r Citep("zhao")`.
...
This model generates more realistic data than any parametric model that I know of.
...
1. Estimate `\hat{\gamma}, \hat{\beta}\left(\tau\right)` from real data. 
	1. This defines `$\hat{F}_{y \vert x, t, m}$` from which to simulate community profiles.
2. Rank species according to []. 
	1. To define true positives and negatives, we set true effects for all but the top 25% to 0.
3. Simulate data alternative configurations
	1. We vary the sample size and rescale coefficients $\hat{\beta}\left(\tau\right)$
4. Estimate models across configurations and compute power curves

---

### Calibration

1. All of our models are somewhat mis-specified, and if we didn’t correct for it, we would have inflated taxon-level false positive rates
2. However, we can use our simulation strategy to calibrate our inferences
	1. Rank direct and indirect effects
	2. Use simulation to estimate the false discovery proportion across thresholds
	3. Use most powerful threshold that controls FDR
...
*Visualization*
1. Data visualization is like cartography — by reducing what is shown in a display, we can make essential structure clear
2. Like a map, a good visualization can support navigation across unfamiliar territory

*Simulation*
1. Hypothetical reasoning is a central part of statistics, and generative models can make alternative scenarios concrete
2. Generative modeling is useful for more than data analysis — it is a powerful device for experimental design and model evaluation
