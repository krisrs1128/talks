...
\begin{align}
\bar{\tau} &= \Earg{Y\left(X, 1, M\left(X, T\right)\right) - Y\left(X,0, M\left(X, T\right)\right)},\\
\bar{\delta} &= \Earg{Y\left(X, T, M\left(X, 1\right)\right) - Y\left(X,T, M\left(X, 0\right)\right)},
\end{align}
...
\begin{align*}
 \logit{\Parg{Y > 0 \vert X}} = \gamma^{T}X \\
 Q_{Y}\left(\tau \vert X, Y > 0\right) = \xi\left(\tau\right)^{T}X
\end{align*}
