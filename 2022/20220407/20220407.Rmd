...
## Revisiting Iterative Data Structuration: Alignment, Refinement, and Simulation
...
* New technologies are giving a clearer view
...
* Nuisance variation
  - Sequencing protocol (batch effects)
  - Sequencing depth
...
.pull-left[
* Iterative data structuration is the idea of forming sequence of increasingly
realistic models 
* It bridges low and high-complexity descriptions
* Statistical analog of the Focus-plus-Context principle in data visualization
]
...
.pull-left[
* Breaks larger microbiome data analysis problem into more manageable components
  - Begin with the overview
  - Then focus-in and explain residual variation
* An alternative to one-size-fits-all modeling packages
* Approach is common in regression, but is oddly absent from ‘omics
  - Requires complex generative mechanisms
]
...
* At this point in the microbiome workflow, clustering is popular
...
* If the data actually follow a gradient, the clustering is artificial. A more parsimonious description uses either
...
How can we formalize this heuristic? Our ideal approach would support,
  * Navigation: Different $K$ allow us to trade off between richness and
  interpretability
...
* Refinement: Reflects number of strong ancestors
...
.pull-left[
refinement $\uparrow$ but coherence $\downarrow$
```{r}
...
.pull-right[
coherence $\uparrow$ but refinement $\downarrow$
```{r}
...
* Count data dimensionality reduction is an admittedly narrow problem… perhaps not your area?
* Iterative data structuration is a general problem solving device, though
* Let’s look at a few examples from _Generative Models: An Interdisciplinary Review_ (preprint available on request)
- Many more examples there: Experimental design, longitudinal data, likelihood-free inference, …

---

### Mixture Modeling

Suppose we wanted to fit a model to this (simulated) data
...
* A natural enough starting point is to begin with a Gaussian mixture model. I
set $K = 4$.
* We can simulate from the fit. Visually, it is quite far off.
