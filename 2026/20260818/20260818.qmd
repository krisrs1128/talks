...
Modern biomedical data have many measurements per sample, and dimensionality
reduction methods help create simpler representations useful for visualization
and downstream analysis.

![](figures/samples_by_microbes.png){width=1200}

They can summarize microbiome community structure [@Sankaran2018].
...
![](figures/brain_parcellation.png){width=1200}

They are also used to identify brain regions with similar voxel patterns [@Schell2026].
...
- Nonlinear embeddings can distort neighborhoods in a way that biases downstream interpretations.
...
* Topics: $\beta_{k}$ describes $K$ latent topics.
* Memberships: $\gamma_{i}$ describes supervoxel $x_i$ as a mixture of topics.
...
### Example: COPD Subtypes

[@pmlr-v149-saeedi21a] use a version of topic models adapted to continuous
imaging data. Each patient's lung is a Dirichlet mixture of latent tissue
patterns, but the final step is a Gaussian rather than multinomial likelihood.

![](figures/lung_subtypes_memberships.png){width=1000}

---

### Choice of $K$

Analyzing the held-out likelihood suggests that $K \approx 10$ is enough, but
it's unclear which of the recovered patterns are sensitive to this choice.

![](figures/subtypes_test_ll.png){width=1000}
...
An alignment is a graph where nodes $V$ represent topics $\beta\left(v\right)
\in \Delta^D$ and memberships $\Gamma\left(v\right)  =
\left(\gamma_{ik}^{m}\right)_{i=1}^{n} \in \reals^{N}$ across models.
...
* Coherence: Average similarity of $\beta\left(v\right)$ to other topics along the same path. High coherence $\to$ persistent structure.
...
To distinguish between nuclei in the thalamus, [@Yan2023] gathered 63 features
per voxel and applied UMAP to reduce those features to a low-dimensional
embedding.

![](figures/thalamus_umap.png){width=800}

Manual labels were propagated across neighbors in the UMAP space. But can we
trust these?
...
Both $t$-SNE and UMAP introduce distortions. For example, they can fail to
preserve the topology of the underlying data.
...
They also may not preserve density within different regions of the plot.
...
impact scientific interpretation [@Liu2025; @Kobak2021; @Kozlov2024; @simplystatisticsSimplyStatistics]. For example, they
create misleading separation between neighborhoods that are actually similar.
...
ggplot(kappa) +
    ...
    labs(x = "Log Condition Number", y = "Count", fill = "Cell Type") +
    facet_grid(method ~ .)
```
...
**Papers**: [https://go.wisc.edu/e2c16h](https://go.wisc.edu/e2c16h), [https://go.wisc.edu/tify36](https://go.wisc.edu/tify36)
...
For each vertex $v$, select the highest-weight incoming edge:
$$e^\ast(v) = \arg\max_{e:\,\text{target}(e)=v}\bigl[\tilde{w}_{\text{out}}(e)+\tilde{w}_{\text{in}}(e)\bigr].$$

Iterating from largest to smallest $l$ yields a set of vertex-disjoint paths,
the number of such paths characterizes the alignment.

![](figures/refinement-branches-1.png){width=90%}
...
![](figures/refinement-branches-2.png){width=90%}
...
![](figures/refinement-branches-3.png){width=90%}
...
![](figures/refinement-branches-4.png){width=90%}
...
![](figures/refinement-branches-5.png){width=90%}
...
![](figures/refinement_diagnostic_example.png){width=390}
...
In the sphere example, these derivatives can be directly computed to obtain,
...
:::: {.columns}
::: {.column width="50%"}
The local isometry visualization highlights some "threads" that are more spread
