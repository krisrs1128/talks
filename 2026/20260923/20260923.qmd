...
make comparisons by looking at large-scale molecular readouts.
   ...
   * In single cell eQTL studies, we might look for cell-type specific gene expression that differs across genotypes.
   ...
   timepoints in a time course experiment) and may want to account for that.
...
This is the differences in empirical means of groups A and B,

$$
\begin{align}
\bar{y}_{A} = \frac{1}{n_{A}}\sum_{i \in \text{ Group } A}y_{i} \\
\bar{y}_{B} = \frac{1}{n_{B}}\sum_{i \in \text{ Group } B}y_{i}
\end{align}
$$

adjusted according to an empirical estimate of the within-group variance,

$$
\hat{\sigma}^2 = \frac{\sum_{i \in A}(y_i - \bar{y}_A)^2 + \sum_{i \in B}(y_i - \bar{y}_B)^2}{n_A + n_B - 2}.
$$

The denominator ensures that we account for the spread within each group when
interpreting the difference in means.

5. We will say that when the means of the two groups are equal, the "null
...
6. Suppose that the null hypothesis $H_{0}$ holds, like the first figure in the
pair above. In this case it can be theoretically shown that the statistic $t$
has a $t$-distribution with an closed-form formula. Here I've simulated many
datasets where $H_{0}$ is true and overlaid the formula.  The distribution of
$t$ below is called the "null distribution."

   [plot of sampling distn + theoretical fmla]

7. On the other hand if there is a real difference in the group means, then the
observed $t$ would breakaway from this "null" distribution. Here I generated one
sample where the true group difference is 2 and I've overlaid the t-statistic on
that one dataset, on the null from before. This suggests the procedure that
underlies all of hypothesis testing. If we see a statistic that's very unlikely
under the null distribution, then we have evidence that the null hypothesis is
false. The p-value is defined as the area of the distribution above the test
statistic, and it is a measure of the strength of this evidence (smaller area ->
less likely to come from the null).

8. So if we're willing to believe the Gaussian assumption, we have a full
...
figure below, where we care about the difference between A and B but need to
account for differences in some sample-specific factor $X_i$.

   (picture of two parallel lines, slope depending on X)

9. In this case we might want to test the null hypothesis $H_{0}$ that the
lines for the two groups are exactly overlapping. The main trick is to subtract
out the effects from the variable X and see whether there still remains a
difference between the two groups. That is, after subtracing the $X$ effect, we
return to the setting above. This is exactly what a linear model does. The
formula becomes a bit more complicated, but we can again derive a statistic $t$
that has a t-distribution under $H_{0}$.

10. The fact that we can make comparisons between groups while accounting for
other variables is what makes linear models such a versatile statistical method.
My concrete example has used a straight line for X but in fact the same
machinery works for nonlinear time courses, when the slopes of the lines are not
necessarily parallel, and when we want to compare more than two groups at once.

## Normalization

11. Unfortunately the Gaussian assumption doesn't make any sense for genomics
...
12. There are two significant issues that stand in the way of simply making the
...
13. An idea that solves both problems at once is to apply a log
...
14. Here are histograms of three genes before and after applying the CPM
transformation.
   [code to generate these figures]

15. One problem is that when we convert to compositions we're actually losing a
...
16. The limma-voom workflow addresses this by storing not simply the log CPM
...
17. It turns out that these weights can be incorporated into linear models. For
...
18. At this point we could imagine applying a linear model to every single
...
19. This doesn't quite work, though. It's easiest to understand through a small
...
20. I've plotted the t-statistics vs. the gene index, and I've shaded in those
...
21. We somehow need a better way of estimating these variances. The insight of
...
22. There's one last statistical consideration. If we use the usual hypothesis
...
23. To deal with this phenomenon Benjamini and Hochberg had the idea that
...
24. Formally let $H_{1}, \dots, H_{J}$ represent the null hypothesis for the $J$
...
25. In addition to introducing this quantity, Benjamini and Hochberg proposed a
...
26. This procedure actually has an intuitive explanation in terms of the histogram of p-values.
