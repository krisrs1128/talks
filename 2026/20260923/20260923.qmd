...
In practice, the moderated $t$-statistics are much less affected by the small
standard deviations $s_{g}$.  When using these $t$-statistics instead of the
original ones, we have many fewer false positives among our top-ranked genes.

<img src="figures/09_fraction_null_top_k_moderated.png" width=600/>
...
samples <- data.frame(sex = factor(pickrell1.eset$gender, levels = names(pal_sex)), row.names = colnames(counts))
genes   <- data.frame(gene_id = rownames(counts), symbol = annotEnsembl63[rownames(counts), "Symbol"],
                      chr = annotEnsembl63[rownames(counts), "Chr"], row.names = rownames(counts))
