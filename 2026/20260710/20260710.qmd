...
But sometimes visualizations themselves can be misleading.

![](figures/GTAI_average_hours.gif)
...
Stacked bar pots are a basic visualization used in Microbiome research.

- $x$-axis position encodes samples
- Color encodes taxonomy

We can use this to compare composition across groups or over time.
...
We can collapse and expand the tree and the stacked bar plot rectangles merge or split accordingly.
...
{{<video figures/bar-search-trim.mp4>}}
...
For example, when we analyzed the COVID-19 immunology data from @suetal,
analyzing cell compositions at multiple scales showed us that $\gamma\delta$
cells were only present in patients with mild symptoms.
...
- Interactivity allows us to see the equivalent of hundreds of static views easily.
- Computers make this easy but we shouldn't take it for granted!
