...
Modern omics data have many features, and dimensionality reduction methods are often used to create overview visualizations.

![](figures/samples_by_microbes.png){fig-width=100%}

They are often used to summarize microbiome community structures.
...
![](figures/cell_by_genes.png){width=100%}
...
But diagnostics are limited.

---

### Outline

We describe two new diagnostic visualizations.
...
::: {.column width="40%"}
::: {.smaller}
RMetric plots for t-SNE and UMAP embeddings.
![](figures/swiss_roll_boxplot.gif){width=710}
:::
:::
::::

Both are driven by the same underlying interactive visualization perspectives, reviewed next.
...
![](figures/doitree_dmoz.gif){width=800}

---

### Focus-plus-Context: Barplots

Stacked barplots help visualize sample-to-sample variation in microbiome community structure. They don't give a good way of focusing on finer taxonomic resolutions.

![](figures/composition_barplot_microbiomeviz.png){width=900}

::: {.smaller}
Figure from the microbiomeViz documentation [@microbiomeviz].
:::
...
::: {.smaller}
Figure from the Qiime Viewer documentation [@qiimeviewer].
:::
...
We can apply the focus-plus-context principle to visualize abundances of rare taxa without losing context [@Kuo2025].

![](figures/phylobar_overview.png){width=900}

[Link](https://mkdiro-o.github.io/phylobar/articles/hfhs.html#htmlwidget-ac96cb3ee4656e2e9ec3)

---

## Topic Alignment

paper: [go.wisc.edu/tify36](https://go.wisc.edu/tify36/)
...
![](figures/topic_double_triangle.png)
...
* Define the transport cost $C\left(v, v^\prime\right) := JSD\left(\beta\left(v\right), \beta\left(v^\prime\right)\right)$, the Jensen-Shannon divergence between the pair of topic distributions [@https://doi.org/10.48550/arxiv.1803.00567].
...
Like in the antibiotics study, we visualize change in topics over time. This
highlights smooth transitions in topic memberships.

![](figures/topic_trajectories_pregnancy.jpeg){width=700}
...
![](figures/alto_sketches_annotated alignment.png){width=700}

:::: {.columns}
::: {.column width="50%"}
[Paper](https://go.wisc.edu/tify36)<br/>
![](figures/qr-paper.png){width=80}
:::
::: {.column width="50%"}
[Package](https://lasy.github.io/alto)

![](figures/qr-package.png){width=80}
:::
::::
...
preprint: [https://go.wisc.edu/oe3g62](https://go.wisc.edu/oe3g62)

---

### Challenge

Embedding $\mathbf{R}^{D} \to \mathbf{R}^{2}$ cannot preserve metric structure
in general. The question is:

**Which distortions occur, and where?**
...
:::
:::{.column width="20%"}
See also [@simplystatisticsSimplyStatistics; @Kozlov2024].
:::
::::
...
Let $z_{k} \in \mathbf{R}^{N}$ be the $k^{th}$ embedding dimension. The
embedding map $\varphi : M\to \mathbf{R}^{D}$ induces a pullback metric. The
...
visualization tutorial from the scanpy package [@scanpytutorialsAnalysisVisualization]. Each point is the UMAP embedding of a
...
1. Distortion in nonlinear dimensionality reduction is inevitable, but can be
characterized.

1. Interactivity can reveal distortion information based on on the analysts'
priorities.

**Preprint**: [go.wisc.edu/oe3g62](https://go.wisc.edu/oe3g62)

**Package**:

- [https://github.com/krisrs1128/distortions](https://github.com/krisrs1128/distortions)
