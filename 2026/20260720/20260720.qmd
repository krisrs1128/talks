---
title: "An Introduction to Shapley Values in Biology"
subtitle: "CGSI Tutorial 2026"
...
_Readings: [1](https://arxiv.org/abs/2207.07605), [2](https://proceedings.mlr.press/v108/janzing20a.html), [3](https://proceedings.neurips.cc/paper_files/paper/2024/hash/084252ec5f05f13bf565843c1873686d-Abstract-Conference.html)_; _[Source](https://github.com/krisrs1128/talks/blob/master/2026/20260619/20260619.qmd); Colab: [1](https://colab.research.google.com/drive/1vsdIELDbONhZPPNnNR2Q7MSLBLmXdSC2?usp=sharing), [2](https://colab.research.google.com/drive/1bDTP8cas2EkHqx20KEjiCZ3z-mHAv0g4?usp=sharing)_

## CGSI Motivation

1. On Friday, Prof. Na Cai described an approach to making EHR-based GWAS more
reliable [@Di2026]. A part of the method used a multilayer perceptron model,
EDGAR, to predict deep phenotypes from EHR codes. This is Supplementary Figure 7.

   ![](figures/edgar_shap.png){width=90%}

1. At [CGSI 2024](https://www.youtube.com/watch?v=iWmhAycjEcI), Prof. Christina
Leslie presented SCARlink [@Mitra2024], a model to predict gene expression from
chromatin accessibility data, which can be used to identify cell-type specific
enhancers.  This is from their introduction:
...
1. So, what exactly are Shapley values???
...
**Approach.** SHAP values satisfy all three requirements. This handout focuses
on their theory (Section 1), computation (Section 2), and software (Section 3).

# Conceptual Foundation

## Local Feature Attributions

1. _High-stakes decisions._ When an individual is given a medical diagnosis, knowing the globally most important features isn't enough. They deserve an explanation specific to their case.

   - A related use case is _algorithmic recourse_. What could a stakeholder change to reverse a decision? (e.g., are there any lifestyle changes that might help delay disease symptoms?)

1. _Model debugging._ A model might classify a radiograph $x$ as COVID-19 positive because it has the watermark of a hospital with disproportionately many COVID-19 patients [@DeGrave2021]. This means the model has learned a "shortcut" and won't generalize well -- a patient with the disease but scanned with a different machine would get misclassified.

   ![](figures/covid_shortcut.png){width=60%}
...
we can substitute it into @eq-shapley to get a local feature attribution for
sample $x$.
...
1. Some researchers [@janzing20a] have argued that point (4) in the
...
1. Let's first see why naive SHAP computation is slow. Recall the key quantities,
   ...
   * Feature $d$'s attribution,
   ...
   outside of $S$ removed. We covered three removal strategies:
     ...
     - Marginal^[Compared to the earlier discussion, we're allowing for background subsamples B < N.]: $v(S) = \mathbb{E}_{p(X_{\bar{S}})}[f(x_S, X_{\bar{S}})] \approx \frac{1}{B}\sum_{i=1}^{B} f(x_S, x_{i,\bar{S}})$
...
1. _Antithetic sampling_. Recall that
   ...
   $$
   so weakly correlated variables have smaller variances when added.
   ...
   so again smaller pairwise correlations reduce variance.
...
bypass enumeration over subsets. For example, it is straightforward to show that
...
the number of trees `nrounds` and learning rate `eta`, selecting
...
\varphi_0 = \text{logit}^{-1}\mathbb{P}(y \vert x_i)$. Most feature importance
