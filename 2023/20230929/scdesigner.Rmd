...
ggsave("gaussian_fit.png", dpi = 200)

plot_corr(exper, samples)
ggsave("pairwise_cors_t.png", dpi = 200)
```

```{r}
links <- list(
  mu = ~ ns(Age, 3) * Genotype,
  sigma = ~ ns(Age, 3) * Genotype,
  df = ~ Genotype
)
simulator <- setup_simulator(exper, links, ~ StudentTLSS()) |>
  estimate()
samples <- simulate(simulator)
```
...
plot(simulator, "Age", samples, color = "Genotype", transform = identity, feature = features) +
  labs(x = "Mouse Age (Weeks)", y = "Gene Expression (Scaled)")
ggsave("student_fit.png", dpi = 200)
```


```{r, fig.width = 5, fig.height = 5}
plot_correlations(exper, transform = identity)
plot_correlations(samples, transform = identity)
plot_corr(exper, samples)
ggsave("pairwise_cors_t.png", dpi = 200)
```
...
plot(simulator, "Age", samples, color = "Genotype", transform = identity, feature = nulls) +
  labs(x = "Mouse Age (Weeks)", y = "Gene Expression (Scaled)")
ggsave("nulls_unaltered.png", dpi = 200)
```

```{r, fig.width = 7, fig.height = 3}
plot_altered(simulator, nulls, ~ ns(Age, 3))
ggsave("altered_ns.png", dpi = 200)
plot_altered(simulator, nulls, ~ Age)
ggsave("altered_linear.png", dpi = 200)
```
