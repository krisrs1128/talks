...
library(scDesigner)
library(gamboostLSS)
opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, cache = TRUE, dpi = 200, fig.align = "center", fig.width = 6, fig.height = 3, eval = FALSE)
...
.pull-left[
Richard McElreath has proposed a recipe to address this.
]
...
Miguel Hernan argues that we can sharpen discussions about causality by forcing critics to propose a DAG.
...
1. Modularity: We should make generative models that are reconfigurable across varied ‘omics data types and experimental designs.
2. Communication: We have the public data resources that can guide the development of precise hypothesis.
3. Interactivity: We should make it enjoyable to evaluate and modify statistical hypotheses.

---

### scDesign

I have been trying put this philosophy into practice by helping build a new version of the scDesign package.
...
Our main idea is to apply tidy design/HCI principles to improve user interaction with multi-omics simulators.
...
#>     feature              family                       link
#>                                     
#> 1       Pyy ZINBI [mu,sigma,nu] mu~ns(psuedotime, df = ...
#> 2      Iapp ZINBI [mu,sigma,nu] mu~ns(psuedotime, df = ...
...
<img src=“figures/pseudotime_margins.png”/>
...
samples <- sample(sim, n_samples = 10, new_data = grid)
plot_correlations(samples)

<img src=“figures/pseudotime_correlation.png”/>

---

### Mutate

(Interactivity) There are many reasons we might want to alter an initial simulator,
...
<img src=“figures/altered_linear.png”/>_
...
<img src=“figures/hd_effect.png”/>_
...
<img src=“figures/hd_no_effect.png”/>

---

### Mutate

If we want to simulate under a new experimental design, we can specify the new design points when sampling.

new_design <- expand_colData(simulator, c("pseudotime", "cell_type"))
samples <- sample(simulator, n_samples = 10, new_data = new_design)
...
<img src=“figures/simulator_join_conditioning.png”/>_
...
#>      feature              family                       link
#>                                      
#>  1  AK123759 ZINBI [mu,sigma,nu] ~cell_type + UMAP1 + UMAP2
#>  2    ADAM33 ZINBI [mu,sigma,nu] ~cell_type + UMAP1 + UMAP2
#>  3      NFIX ZINBI [mu,sigma,nu] ~cell_type + UMAP1 + UMAP2
```
...
(Modularity) We want a unified interface that works across diverse data types. One way to create consistency is to **augment** the general `SummarizedExperiment` with domain-specific structure .
