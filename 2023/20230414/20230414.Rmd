...
read_csv("figures/summary_table_fdp.csv") |>
  pivot_wider(names_from = c("method", "lag"), values_from = FDP) |>
  kable()
```
...
* This trains on all samples then reconstructs the trajectories starting in the
...
These are out-of-sample forecasting errors. We train on only half of the participants.
...
These are the subject-level data corresponding to a few different trajectory patterns ("sustained decrease, transient increase, sustained increase").
...
Defining counterfactuals and computing mirror statistics:

```{r, eval = FALSE, echo = TRUE}
ws <- steps(c("birth" = TRUE), lengths = 2:4, L = 4)
staxa <- select_taxa(ts, ws[[1]], ws[[2]], \(x) mbtransfer(x, 3, 3), n_splits = 20)
```
