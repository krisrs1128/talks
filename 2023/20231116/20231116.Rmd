...
But what representations help us reason about the models themselves?
]
...
Using the language of `r Citep(bib, "tallec2018can")`, when $z_{t}$ is small, we <span style="color:#9d648e">accelerate time</span> and forget more quickly. When it is
large, we <span style="color:#5b8ac0">expand time</span> and preserve memory.
...
In our trained model, we can analyze the $z_{t}$ to see how our counting model warps time.
...
* **Linear Classifier Probes**: To see whether a layer has learned particular "concept," try predicting using its activations to learn a linear classifier `r Citep(bib, "Alain2016UnderstandingIL")`.
...
.center[
<img src="figures/scgpt-atlas.png" width=540/>
]
