...
1. The Handelsman and Davidson Labs designed a longitudinal study to evaluate
the relationship between mindfulness and the microbiome. The treatment group
received meditation training.
...
.pull-left[
\begin{align*}
Y &\sim \Mult\left(n, \varphi^{-1}\left(X^{T}\beta\right)\right) \\
...
\begin{align*}
Y &\sim \Mult\left(n, \varphi^{-1}\left(\mu\right)\right) \\
...
1. We compare models using a semisynthetic simulator, following `r Citep(bib, "ling2021powerful")`.
...
* Let's see how simulation strategies compare on a simple setup.
...
.pull-left[
* Based on the simulation results, we use a diff-in-diff version of the LNM.
