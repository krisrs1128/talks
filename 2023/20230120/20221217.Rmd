...
3. Review the more general literature on evaluation of causal inference models.
...
1. We were concerned that the mindfulness intervention might be affect
behavioral factors that in turn influences microbiota composition.
2. To address this, we developed a suite of mediation analysis approaches,
following work that generalizes mediation analysis to nonlinear settings.
...
.pull-right[
```{r, fig.height = 6, fig.width = 5, out.width = 320}
...
1. The basic difficulty that these papers address is that there is a
bias-variance trade-off in causal inference methods as well.
2. The richest model won’t always give us the best counterfactual estimates (as
...
.center[
<img src="figures/complexity_tradeoff_ite.png" width=600/>
]
...
<img src="figures/candidate_f.png"/>
]
...
and $y_{i}^{0}$. It simultaneously learns to simulate  $x_i$, $t_{i} \vert x_{i}$, and $y_{i} \vert x_{i}, t_{i}$.
...
1. So far, I have only considered simulation-based approaches. But there is an
interesting alternative that has a very different flavor.
2. The main idea is to compare the model’s predicted treatment effect with some
nonparametric estimate on a validation set.
...
counterfactual / hypothetical reasoning), but methods for better model
