...
* **Biological Understanding**: Generative models can suggest improved mental models of biological processes.
...
solve problems we may not have even anticipated.
...
For the marginals, we can borrow from existing LSS packages (gamlss `r Citep(bib, "Rigby2005GeneralizedAM")`, gamboostLSS `r Citep(bib, "Hofner2014gamboostLSSAR")`) to implement a variety of gene-wise regressions,

* Gaussian, Gamma, Poisson, Binomial, Negative-Binomial, Zero-Inflated Poisson, Zero Inflated Negative Binomial,…

We can also build a unified interface for a variety of copula estimation routines,
...
samples <- sample(simulator, new_data = grid)
...
We can change the regression formula for specific genes. Syntax color highlights modifications that haven’t been re-fit.
...
altered <- mutate(margins, Spp1:Cck, link = ~ pseudotime) |>
  estimate(sce)
...
margins |>
  mutate(Spp1:Cck, link = ~ pseudotime) |>
  mutate(c("Pyy", "Ins1"), family = ~ GaussianLSS()) |>
