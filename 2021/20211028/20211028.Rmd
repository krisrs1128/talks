...
library(knitr)
set.seed(102821)
opts_chunk$set(echo = TRUE, fig.align = "center", cache = TRUE, fig.width = 5, fig.height = 5)
library(ggplot2)
...
library(scico)
scale_fill_continuous <- function(...) scale_fill_scico(..., palette = "lapaz", direction = -1)
scale_color_discrete <- function(...) scale_color_brewer(..., palette = "Set2")
...
```{r, echo = FALSE, fig.cap = "What would be a bad choice of logistic function?"}
...
* $g^k, \*h^k$: Nonlinearity and hidden units for layer $k$
...
\begin{align*}
\sigma\left(\sum_{k} w^{2}_{1k} h_{k}^{1} + b_{1}^2\right)
\end{align*}

and since the $h_k$'s are themselves sigmoids, the inner summation is like

\begin{align*}
\sum_{k} w^{(2)}_{1k} \sigma\left(w_{k}^{1}x + b_{k}^{1}\right)
\end{align*}
