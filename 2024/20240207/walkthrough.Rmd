...
exper <- SingleCellExperiment(
  assays = list(counts = t(samples[, -1]))
)
colData(exper) <- DataFrame(mapping)
colData(exper)$celltype <- rep("A", nrow(mapping))
...
scdesign_result <- scdesign3(
  ...
  mu_formula = "s(time, k = 10, bs = 'cr')",
  sigma_formula = "s(time, k = 5, bs = 'cr')",
  corr_formula = "1",
  trace = TRUE
)
```
...
data.frame(t(scdesign_result$new_count)) |>
  ...
  left_join(mapping) |>
  ggplot() +
  geom_line(aes(time, value, group = subject)) +
  scale_y_log10() +
  facet_wrap(~ reorder(ASV, -value))
```
...
plot_univariate(fit, exper)
```

```{r}
margins <- margins |>
  mutate(link = list(mu = ~ phase, sigma = ~ 1))
margins
```
...
fit <- margins |>
  ...
  estimate(exper)
plot_univariate(fit, exper, line_opts = list(linewidth = 2))
```

```{r}
sample(fit)
samples_df <- sample(fit) |>
  pivot_experiment()
samples_df
```
...
We can get more replicates 10 replicates.
...
dge <- DGEList(counts = assay(yachida), group = colData(yachida)$disease) |>
  calcNormFactors() # normalize across different depths
test_results <- differential_test(dge, colData(yachida))
ggplot(test_results, aes(logFC, -log(adj.P.Val, 10))) +
  geom_point() +
  geom_text_repel(data = filter(test_results,  abs(logFC) > 2), aes(label = ID), size = 3)
```
...
n_per <- 250
new_data <- tibble(disease = factor(c(rep("CRC", n_per), rep("healthy", n_per)), levels = c("healthy", "CRC")))
simulated <- sample(sim, new_data = new_data)

new_dge <- DGEList(counts = 2 ^ assay(simulated), group = colData(simulated)$disease)
new_results <- differential_test(new_dge, colData(simulated)) |>
