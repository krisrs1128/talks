---
title: "Methods for Biological Data Workshop"
format:
  html:
    code-fold: true
date: "`r Sys.Date()`"
...
exper <- SingleCellExperiment(assays = list(counts = t(samples[, -1])))
...
```{r}
yachida <- readRDS("https://github.com/krisrs1128/talks/raw/master/2024/20240207/data/yachida.rds")
results <- DGEList(assay(yachida), colData(yachida)$disease) |>
  calcNormFactors() |>
  differential_test(colData(yachida))
...
new_results <- DGEList(2 ^ assay(simulated), colData(simulated)$disease) |>
