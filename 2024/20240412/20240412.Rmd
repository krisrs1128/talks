...
1. Review Gosmann et al. (2017) - vaginal microbiome’s role in HIV acquisition.
...
[Gosmann, Christina et al. “Lactobacillus‐Deficient Cervicovaginal Bacterial Communities Are Associated with Increased HIV Acquisition in Young South African Women.” Immunity 46 (2017): 29–37.](http://dx.doi.org/10.1016/j.immuni.2016.12.013)
...
2. Normalization: Scater, DESeq2 size factors
...
2. Copy notebooks into a GitHub repo and NHGRI AnVIL.
...
Analysis ("DIABLO") after featurizing all data sources to the subject level.

* $x_{i}^{(k)}$: Data from modality $k$, after transformation and featurization to the subject level.
* $y_{i}$: Either HIV acquisition status or number of CD4+ T cells with CCR5 co-receptors.

---

### multiblock sPLS  - Outputs

1. For each modality $k$, subject-level scores $z_{k}$ 
2. For each modality $k$, a sparse subset of features for each PLS dimension.
3. CV performance: Using the subject-level scores, how well can we predict
out-of-sample inflammation/HIV acquisition?
4. Block importances: How much does performance drop when we leave the block
out?
...
(Give a concise algorithmic description)
...
For example, a bacterial taxon (16S data) may increase the levels of an immune cell type (scRNAseq data or cell phenotyping) that is targeted by HIV, ultimately resulting in increased disease risk...The feature pair defines a predictive association rule and hence can be learned by iRF. 
...
That is essentially our plan.h
