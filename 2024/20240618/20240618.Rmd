...
variation. This can often be accomplished using a form of matrix factorization.
...
Methods that assign samples to one of $K$ latent types have this form, where the
memberships are a matrix of one-hot encodings. 

---

### Factor Models

When we relax the requirement $\theta_{ik} \in \{0, 1\}$ to $\theta_{ik} \in
\mathbb{R}$, then we arrive at a form of factor model.

---

### implications

Therefore, if we separately estimate per-sample scores $\theta_{i}$, then we can
use these as covariates to guide simulation.
