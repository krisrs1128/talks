...
Can you begin with some actual examples. The Asthma and Bard examples are nice.

---

### Motivation: Hypothesis Generation

The most important features in a model may guide follow-up experimentation and
theory building.

---

### Motivation: Validation and Improvement

It is worth checking that models are doing well for the right reasons. We can
edit/refine them if not.
...
### Useful Analogy: Data Visualization

1. Both data visualization and model interpretability develop representations that
make it easier to think about information stored in machine (but not human)
friendly formats.

1. In interpretability, we have to think both about the original data *and* a
model trained on it.

---

I'm not sure this is really worth including. I do like the analogy, but seems
potentially like a distraction from the larger computational themes. Not really
information for people to work off of.
...
This is good. We're now getting more specific about techniques, actual problems
solving and data analysis.

Higher-level theme can be: We are good at using computers for problem solving, a
lot of it is automation, but not all...

Make sure to capture peoples' attention early on. Okay to end on a more
philosophical note, but that's not really the key.
...
To be useful, an interactive system needs to make it easy to ask questions and
quickly get answers `r Citep(bib, "Hutchins1985-oz")`: 

1. Gulf of Execution: How long does it take to see the response to a query?

1. Gulf of Evaluation: How long does it take to understand the output?
...
mixed-membership model. Low $K$ gives an overview, large $K$ gives details.
...
Will you introduce that at the beginning? Or will you be only mentioning this at
the end? Tricky because you are trying to cover two themes at once...

For interactivity, it definitely makes sense to include something like this. For
interpretability, it's not quite as clear.

---

### Conclusion

Interactivity and interpretability are both exercises in creating links between
human and machine representations.

1. Embeddings, perturbations, and concepts are useful for answering different
types of questions related to complex models.

1. The interaction gulfs, linking, and focus-plus-context are useful principles
to keep in mind when designing any interactive software.
