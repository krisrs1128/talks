...
1. LIME and Shapley are used when we want to understand the important variables for predictions at a specific instance $x$.
...
1. One idea is to consider the values of $x_{d}$ that were observed in our
dataset. Then, we can evaluate our model over a range of values $x_{-d}$ after
fixing those values of $x_{d}$. 

1. These curves are called Ceteris Paribus ("All Else Held Equal") profiles.
...
There is a subtle problem: If $x_{d}$ and $x_{d'}$ are highly correlated, we
...
To resolve these issues, consider the derivaties,
...
(add a picture to summarize this)
...
\begin{align*}
g_{d}\left(x_{d}\right) := \int \Esubarg{x_{-d}\vert x_{d}}{\left.\frac{\partial f\left(x_d, x_{-d}\right)}{\partial x_d} \right| x_{d} = z} dz
\end{align*}
...
\begin{align*}
\sum_{k = 1}^{K} \frac{1}{N_{d}\left(k\right)} \sum_{x_{i} \in N_{d}\left(k\right)} \left[f\left(z_{dk}^{k}, x_{-d}\right) - f\left(z_{d,k-1}, x_{-d}\right)\right]
\end{align*}

(add the picture with the extended endpoints)
...
measure proposed in `Citep(bib, "")` for decision trees and random forests.
...
\sum_{x_{i} \in t} \left(y_i - \overline{y}_{t}\right)^2 -
\sum_{x_{i} \in t_{L}} \left(y_i - \overline{y}_{t_{L}}\right) ^ 2 - 
\sum_{x_{i} \in t_{R}} \left(y_i - \overline{y}_{t_{R}}\right) ^ 2 \right]
\end{align*}
...
To go from decision trees to random forests....
...
1. E.g., `Citep(bib, "")` argue that using $R^2$ from a ridge regression model
leads to more stable rankings than ordinary MDI.
