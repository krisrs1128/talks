...
(take material from your course)
...
\begin{align*}
g_{d}\left(x_{d}\right) := \int \Earg{\frac{\partial f\left(x_d, x_{-d}\right)}{\partial x_d} \vert x_{-d} = z} dz
\end{align*}
...
\begin{align*}
\sum_{k = 1}^{K} \frac{1}{N_{d}\left(k\right)} \sum_{x_{i} \in N_{d}\left(k\right)} f\left(z_{dk}^{k}, x_{-d}\right) - f\left(z_{d,k-1}, x_{-d}\right)
\end{align*}
...
Each neighborhood $N_{d}$ is usually relatively small. This means that ALE plots
needs fewer function evaluations $f$ compared to Partial Dependence Plots.
