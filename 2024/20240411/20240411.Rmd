...
With a given sample size and experimental design, what types of signals will be recoverable?
  ...
  * This guards against wasting effort on underpowered studies, and is required by almost every NIH grant proposal.
  * In practice, researchers often report power for tests with analytical power curves (e.g., $t$-tests), even if this is not the intended analysis!

We can use simulation to guide power analysis that reflects realistic data analysis.
...
<!-- * Use Tandem Mass Spectrometry to measure abundances of human and microbiome-derived proteins in stool samples -->
.pull-left[
* Sample participants at several stages of T1D progression (33 with T1D, 17 at high risk, 29 at low risk, 22 healthy)
* Analyzed with sparse Partial Least Squares Discriminant Analysis (sPLS-DA), a variant of linear discriminant analysis.
]
...
Their findings suggest that intestinal and pancreas function are altered even before the onset of full T1D.
...
.pull-left[
* We `mutate` the simulators so that the true number of T1D-associated proteins is $s \in \{10, 30, 50, \dots, 110\}$
...
* GAN precision-recall measures can offer a more systematic approach.
]
...
<!-- scDesigner offers freedom, but no guidance. -->
...
* Can we "recommend" the next step of simulator construction, based on initial user specification?
