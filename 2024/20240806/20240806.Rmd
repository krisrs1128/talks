...
We are only beginning to understand microbial community dynamics. This knowledge
is critical for moving from description to design `r Citep(bib, c("2024", "Tian2022"))`.
...
1. **Transfer Functions**: Model intervention and interaction effects across
time horizons using a high-dimensional linear model.

1. **Mirror Statistics**: Use agreement across data splits to allow false
discovery rate-controlled inference of perturbation effects.

<img src="figure/approach.png"/>
...
.pull-left[
Classical transfer function models are autoregressive models:
...
```
<span style="font-size: 18px;">
In `r Citep(bib, "box1975intervention")`, $y_{t}$ are ozone concentrations and $w_{t}$ are new regulations.
 </span>
...
1. Note that, unlike generalized Lotka-Volterra models, they can model lagged
...
1. We will want test statistics that are sensitive to effects that are not
easily summarized by individual coefficients.

---

### General Approach: Mirror Statistics

Liu et al. `r Citep(bib, "Dai2022")` splits the data into $\mathcal{D}^{(1)} = \left(\mathbf{X}^{(1)}, \mathbf{y}^{(1)}\right)$ and $\mathcal{D}^{(2)} = \left(\mathbf{X}^{(2)}, \mathbf{y}^{(2)}\right)$ and check for agreement in the estimates $\hat{\mathbf{\beta}}^{(1)}$ and $\hat{\mathbf{\beta}}^{(2)}$ across splits.
...
To choose a significance threshold $t$, estimate the false discovery rate:
...
For nonlinear effects, we first randomly split series into subsets, $\mathcal{D}^{(1)}$ and $\mathcal{D}^{(2)}$. We estimate models $\hat{\*f}^{(s)}$ using $\left(\*Y_{t}^{(i)}, \*W_{t + 1}^{(i)}, \*z^{(i)}\right)$ for subjects $i$ in split $s$.

---

### Partial Dependence

Next, we estimate the partial dependence `r Citep(bib, c("Friedman2001", "JMLR:v19:18-416"))` of the intervention $\mathbf{W}_{t}$ across time horizons.
...
Our definition uses $PD_{j}^{(s)}$ as analogous to the usual $\hat{\beta}_{j}$.
...
1. Inference: How do methods compare in their FDR control and power?

1. Forecasts: How do methods compare in their ability to predict short and
long-term community response to environmental interventions?
...
<img src="figure/dimensionality.png" width=50/> Number of (nonnull) taxa: Dimensionality and the signal enrichment varies across applications.

<img src="figure/normal_curve.png" width=80/> Signal strength: Both inference and forecasting are affected by the strength of the intervention.
...
Negative binomial models are often good fits to microbiome data `r Citep(bib, c("Zhang2017", "Jiang2019", "Mishra2022"))`. We use an
...
1. `fido` `r Citep(bib, c("Silverman2018", "JMLR:v23:19-882"))`: A multinomial regression model for count data that we adpted for forecasting.
...
<span style="font-size: 18px;">
Out-of-sample performance is strongest among highly abundant taxa (top row) and
short-term time lags (left column).
</span>
...
Estimates $PD_{j}$ (top panel) and underlying data (bottom panel) for taxa significant effects.
</span>
...
<img src="figure/phylogenetic_covariance.png"/>
