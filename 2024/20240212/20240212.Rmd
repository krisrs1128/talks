...
Another common technique is to analyze linear interpolations in this space (ref. Latent Space Cartography).
Here, I have traced out the microbiome profiles lying between two groups.
...
.center[
  <img src="figures/distillation_gams.png"/>
]
...
slightly better performance than if we had fit the GAM originally (just remove
this slide if not true). 
...
$$
\left(x_{i} - x_{i}') \int_{\alpha \in \left[0, 1\right]} \frac{\partial f\left(x_{i}' + \alpha\left(x_{i} - x_{i}'\right))}{\partial x_{i}} d\alpha
$$
...
.center[
  <img src="figures/integrated_gradients_animation.gif"/>
]
...
.center[
  <img src="figures/sanity_checks.png"/>
]
...
.center[
<img src="figures/koh_concept.png"/>
]
...
We can train this model using tuples $\left(x_{i}, c_{i}, y_{i}\right)$ and
objective function:
...
Performance is comparable to before, and we obtain intermediate concept labels
to explain each instance's prediction.
...
An open challenge in interpretability is how to work with multiple sources
simultaneously. 
