...
For human welfare, it's important to create mechanisms for interpreting and
having agency over these models.
...
The nuance with which we can think about visualization can be translated to interpretability.
...
that are nearby in the embedding space have similarly shaped trajectories and
community contexts.
...
Latent Space Cartography). Here, I have traced out the microbiome profiles lying
between two groups.
...
standard Data $\to$ Model $\to$ Loss workflow.
   - Is it using features that will generalize to new contexts?
...
.pull-left[
It's possible to create both realistic and simplified tasks and measure how
system (Human + AI) level performance is affected by model explanations.
]

.pull-right[
<img src="figures/human_ai.png"/>
]
...
.pull-left[
A direct approach to evaluating local explanations is to see whether it helps
study participants to edit instances in a way that changes their prediction.
]
