...
We have begun a collaboration with researchers who study the human gut
microbiome -- the ecosystem of microorganisms that live in the gut. Like in
ordinary ecology, they are curious about:
...
.pull-right[
```{r, fig.cap = "The microbiome along the gut lining, from Earle et al. 2015.", fig.align = "center"}
...
to nutrition. The want to compare the microbiomes of people with low and high
fat diets. 
...
* The budget should be used as wisely as possible
...
1. We know that many methods have been proposed for microbiome differential
abundance testing.

1. Standard $t$-tests are insufficient because:

* The original data are sparse and far from normal
* Some samples have been sequenced more deeply than others

So we can't rely on $t$-tests power calculators.

---

### Approach: Simulation

Instead, we can consider simulation. We will simulate experiments and see what
our power and false discovery rate (FDR) look like in those hypothetical
datasets.

This transforms abstract power calculation $\to$ concrete simulation experimentation.

---

### Simulation using Templates

* Instead of simulating data from scratch, it's easier to train a generative
model to existing experimental data (``template data'').

* Generative models can _generate_ new, hypothetical samples, not just fit observed ones `r Citep(bib, "Sankaran 2020")`.

---

### Foundation: The Atlas Study

We decide that the most similar dataset to our proposal is the dataset in `r Citep(bib, "")`.
...
`SummarizedExperiment` objects. These are containers that store:

* Feature $\times$ sample measurements in the `assay()` slot
* Feature annotation in the `rowData` slot
* Sample annotation in the `colData` slot
...
<img src="figures/summarized_experiment.svg" width=800/>
