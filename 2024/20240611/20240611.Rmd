...
* Our goal today will be to simulate data that look similar to that data, but where we control the ground truth associations and experimental design.
...
.center[
  <img src="figure/marginal_goals.png" width=700/>
]
...
* E.g., in the coin flipping example, the coin's probability of heads is no longer constant
...
.center[
  <img src="figures/gaussian.svg" width=800/>
]
...
1. On their own, these distributions may not seem like much, but when we allow them
...
1. How many samples are needed for differential abundance analysis using Limma?
1. We use can the Atlas dataset as a template.
1. Main Steps:
  - Estimate a simulator from the template
  - Remove any association for true negative taxa
  - Simulate different sample sizes and evaluate hypothesis test results.

Let's run this in the exercise sheet.
